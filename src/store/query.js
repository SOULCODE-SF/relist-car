let query = {
  ...
  generations: {
    getGenerationByModelQuery: `SELECT g.id, g.title, g.image_path, SUBSTRING(MAX(gi.start_production), 1, 4) as start_production, SUBSTRING(MAX(gi.end_production), 1, 4) as end_production, MAX(gi.body_type) as body_type , MAX(gi.engine) as engine, MAX(CONCAT_WS(' x ', d.length, d.width, d.height)) AS dimension, b.name as brand_name, m.name as model_name, b.id as brand_id, MAX(es.power) as power FROM generations g JOIN generation_links_2 gl on g.id = gl.generation_id LEFT JOIN general_information gi on gl.id = gi.generation_link_id LEFT JOIN dimensions d on d.generation_link_id = gl.id JOIN models m on g.model_id = m.id LEFT JOIN engine_specs es on es.generation_link_id = gl.id JOIN brands b on b.id = m.brand_id WHERE g.model_id = ? GROUP BY g.id;`,
  },
  banners: {
